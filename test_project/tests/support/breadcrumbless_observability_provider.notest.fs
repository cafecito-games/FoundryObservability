...
## Event-capable provider fixture that intentionally omits breadcrumbs.
class_name BreadcrumblessObservabilityProvider
extends RefCounted
...
func capture(_event: ObservabilityEvent) -> String:
	if not is_available():
		return ""
	_event_sequence += 1
	return "event:%s" % _event_sequence
